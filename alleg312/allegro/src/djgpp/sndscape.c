...
static int soundscape_detect(int input)
{
   ...
   if (status == FALSE)
      strcpy(allegro_error, get_config_text("Soundscape not found"));
   else {
	 ...
	 soundscape_freq = 48000;
      ...
      sprintf(soundscape_desc, get_config_text("Soundscape %s (%d hz) on port %X, using IRQ %d, DMA channel %d and waveport %X"),
