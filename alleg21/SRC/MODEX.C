...
static BITMAP *modex_init(int w, int h, int v_w, int v_h);
static int modex_scroll(int x, int y);

static BITMAP *xtended_init(int w, int h, int v_w, int v_h);
...
static BITMAP *modex_init(int w, int h, int v_w, int v_h)
{
    ...
    */
   b = _make_bitmap(v_w/4, v_h, 0xA0000, &gfx_modex);
   if (!b)
      return NULL;

   b->w = b->cr = v_w;
   b->vtable = &_modex_vtable;
...
static BITMAP *xtended_init(int w, int h, int v_w, int v_h)
{
   ...
   b = _make_bitmap(640/4, 400, 0xA0000, &gfx_xtended);
   if (!b)
      return NULL;
...
/* _x_textout_8x8:
 *  Draws an 8x8 text string onto a mode-X screen.
 */
void _x_textout_8x8(BITMAP *bmp, FONT_8x8 *f, char *str, int x, int y, int color)
{
   unsigned char *dat;
   unsigned long addr;
   char *s;
   ...
   for (plane=0; plane<4; plane++) {
	 ...
	 if ((x_pos+plane >= bmp->cl) && (x_pos+plane+4 < bmp->cr)) {
	    /* draw both bytes of the char (2 bytes * 4 planes = 8x8 font) */
...
void _x_clear_to_color(BITMAP *bitmap, int color)
{
   ...
   _linear_clear_to_color(bitmap, color);
