...
{
   rect(screen, (SCREEN_W-256)/2-1, 128, (SCREEN_W+256)/2+1, 140, 255);
   rectfill(screen, (SCREEN_W-256)/2, 129, (SCREEN_W-256)/2+pos, 139, 1);
}
...
int check_tables()
{
   if ((!rgb_map) || (!trans_map) || (!light_map)) {
      show_mouse(NULL);
      text_mode(0);

      if (!rgb_map) {
	 clear(screen);
	 textout_centre(screen, font, "Creating RGB table", SCREEN_W/2, 64, 255);
	 ...
	 create_rgb_table(rgb_map, mypal, table_callback);
      ...
      if (!trans_map) {
	 clear(screen);
	 textout_centre(screen, font, "Creating translucency table", SCREEN_W/2, 64, 255);
	 ...
	 create_trans_table(trans_map, mypal, 96, 96, 96, table_callback);
      ...
      if (!light_map) {
	 clear(screen);
	 textout_centre(screen, font, "Creating lighting table", SCREEN_W/2, 64, 255);
...
int spline_proc()
{
   ...
   textout_centre(screen, font, "spline test", SCREEN_W/2, 6, 15);
   textout_centre(screen, font, "Drag boxes to change guide points,", SCREEN_W/2, 64, 15);
   textout_centre(screen, font, "and press ESC to finish", SCREEN_W/2, 80, 15);
...
int polygon3d_proc()
{
   ...
   textout_centre(screen, font, "3d polygon test", SCREEN_W/2, 6, 15);
   textout_centre(screen, font, "Use the arrow keys to rotate the", SCREEN_W/2, 64, 15);
   textout_centre(screen, font, "cube, + and - to zoom, space to", SCREEN_W/2, 80, 15);
   textout_centre(screen, font, "change drawing mode, enter to tile", SCREEN_W/2, 96, 15);
   textout_centre(screen, font, "the texture, and ESC to finish", SCREEN_W/2, 112, 15);
   ...
   for (;;) {
      if (redraw_mode) {
	 rectfill(screen, 0, 24, SCREEN_W, 32, 0);
	 textout_centre(screen, font, mode_desc[mode], SCREEN_W/2, 24, 255);
	 redraw_mode = FALSE;
      }
