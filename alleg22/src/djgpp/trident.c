...
static int trident_scroll(int x, int y)
{
   ...
   if(gfx_trident.vid_mem < 1024*1024)
      addr = (y * VIRTUAL_W + x + 1) >> 2;
   else
      addr = (y * VIRTUAL_W + x + 3) >> 3;
   ...
   /* now the extended Trident-specific regs */

   /* set bit 16 of the screen start address */
   outportb(0x3D4, 0x1E);
   outportb(0x3D5, (inportb(0x3D5) & 0xDF) | ((addr & 0x10000) >> 11) );

   /* bits 17-19 of the start address */
   /* uses the 8900CL/D+ register */
   outportb(0x3D4, 0x27);
