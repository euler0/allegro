...
AF_DRIVER *_af_driver = NULL;          /* the VBE/AF driver */

static int in_af_mode = FALSE;         /* true if VBE/AF is in use */

int _af_active;                        /* true if accelerator enabled */

void *_af_set_bank;                    /* shortcut to bank switch function */
void *_af_wait_till_idle;              /* shortcut to wait idle function */
void *_af_enable_direct_access;        /* shortcut to enable function */
...
static inline int call_vbeaf(void *proc)
{
   ...
   : "b" (_af_driver),                 /* VBE/AF driver in ds:ebx */
...
static inline int set_vbeaf_mode(int mode, int w, int h, int v_w, int v_h, int *width_ret)
{
   ...
   : "b" (_af_driver),                 /* VBE/AF driver in ds:ebx */
     "rm" (_af_driver->SetVideoMode),  /* mode set function in reg or mem */
...
static int load_vbeaf_driver(char *filename)
{
   ...
   _af_driver = malloc(size);

   if (pack_fread(_af_driver, size, f) != size) {
      free(_af_driver);
      _af_driver = NULL;
      return FALSE;
   }
   ...
   _go32_dpmi_lock_data(_af_driver, size);
...
static int initialise_vbeaf_driver()
{
   ...
   for (c=0; c<4; c++) {
      if (_af_driver->IOMemoryBase[c]) {
	 if (_create_linear_mapping(af_memmap+c, _af_driver->IOMemoryBase[c], 
					     _af_driver->IOMemoryLen[c]) != 0)
	    return -1;

	 _af_driver->IOMemMaps[c] = (void *)(af_memmap[c]-__djgpp_base_address);
      }
   }

   /* create mapping for banked video RAM */
   if (_af_driver->BankedBasePtr) {
      if (_create_linear_mapping(&af_banked_mem, _af_driver->BankedBasePtr, 
							       0x10000) != 0)
	 return -1;

      _af_driver->BankedMem = (void *)(af_banked_mem-__djgpp_base_address);
   }

   /* create mapping for linear video RAM */
   if (_af_driver->LinearBasePtr) {
      if (_create_linear_mapping(&af_linear_mem, _af_driver->LinearBasePtr, 
					  _af_driver->LinearSize*1024) != 0)
	 return -1;

      _af_driver->LinearMem  = (void *)(af_linear_mem-__djgpp_base_address);
   }
   ...
   _af_driver->Sel0000h = af_sel0000h;
   _af_driver->Sel0040h = af_sel0040h;
   _af_driver->SelA000h = af_sela000h;
   _af_driver->SelB000h = af_selb000h;
   _af_driver->SelC000h = af_selc000h;

   /* callback functions: why are these needed? ugly, IMHO */
   _af_driver->Int86 = _af_int86;
   _af_driver->CallRealMode = _af_call_rm;
...
{
   unsigned short *mode = _af_driver->AvailableModes;

   /* search the list of modes */
   while (*mode != 0xFFFF) {
      if (call_vbeaf_ax_di(_af_driver->GetVideoModeInfo, *mode, mode_info) == 0) {
...
static BITMAP *vbeaf_init(int w, int h, int v_w, int v_h)
{
   ...
   if (strcmp(_af_driver->Signature, "VBEAF.DRV") != 0) {
      vbeaf_exit(NULL);
   ...
   if (call_vbeaf((char *)_af_driver+(long)_af_driver->InitDriver) != 0) {
      vbeaf_exit(NULL);
   ...
   if (mode_info.Attributes & 0x100)
      gfx_vbeaf.vsync = vbeaf_vsync;
   else
      gfx_vbeaf.vsync = _vga_vsync;

   gfx_vbeaf.vid_mem = _af_driver->TotalMemory * 1024;
   ...
   if (mode_info.Attributes & 8) {
      ...
      gfx_vbeaf.bank_gran = _af_driver->BankSize * 1024;
   }
   ...
   b = _make_bitmap(width, height, gfx_vbeaf.linear ? 
		    (unsigned long)_af_driver->LinearMem : 
		    (unsigned long)_af_driver->BankedMem, &gfx_vbeaf);
   if (!b) {
      vbeaf_exit(NULL);
      return NULL;
   }

   gfx_vbeaf.vid_phys_base = (gfx_vbeaf.linear ? _af_driver->LinearBasePtr : 
						 _af_driver->BankedBasePtr);
   ...
   _af_set_bank = _af_driver->SetBank;
   _af_wait_till_idle = _af_driver->WaitTillIdle;
   _af_enable_direct_access = _af_driver->EnableDirectAccess;
   ...
   sprintf(vbeaf_desc, "VBE/AF %ld.%ld (%s)", _af_driver->Version>>8,
			_af_driver->Version&0xFF, _af_driver->OemVendorName);
...
static int vbeaf_scroll(int x, int y)
{
   if (_af_driver->WaitTillIdle)
      call_vbeaf(_af_driver->WaitTillIdle);
   ...
   : "b" (_af_driver),                    /* VBE/AF driver in ds:ebx */
     "rm" (_af_driver->SetDisplayStart),  /* scroll function in reg or mem */
...
static void vbeaf_set_pallete_range(PALLETE p, int from, int to, int vsync)
{
   ...
   : "b" (_af_driver),                    /* VBE/AF driver in ds:ebx */
     "rm" (_af_driver->SetPaletteData),   /* pallete function in reg or mem */
...
static void vbeaf_exit(BITMAP *b)
{
   ...
   if (in_af_mode) {
      call_vbeaf(_af_driver->RestoreTextMode);
      in_af_mode = FALSE;
   }
