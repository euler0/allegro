...
void *grab_binary(char *filename, long *size, int x, int y, int w, int h)
{
   void *mem;
...
void save_binary(DATAFILE *dat, int packed, int packkids, int strip, int verbose, int extra, PACKFILE *f)
{
   pack_fwrite(dat->dat, dat->size, f);
}
...
{
   return datedit_save_datafile((DATAFILE *)dat->dat, filename, -1, -1, FALSE, FALSE, FALSE);
}
...
void *grab_datafile(char *filename, long *size, int x, int y, int w, int h)
{
...
FONT *import_grx_font(char *fname)
{
   ...
   for (c=0; c<FONT_SIZE; c++) {
	 ...
	 font_prop->dat[c] = create_bitmap(8, hdr.height);
...
FONT *import_bios_font(char *fname)
{
   ...
   for (c=0; c<FONT_SIZE; c++) {
      font->dat.dat_prop->dat[c] = create_bitmap(8, 16);
...
FONT *fixup_font(FONT *f)
{
   ...
   for (c=1; c<FONT_SIZE; c++) {
      if ((f->dat.dat_prop->dat[0]->w != w) ||
	  (f->dat.dat_prop->dat[0]->h != h))
	 return f;
   }
...
void *grab_font(char *filename, long *size, int x, int y, int w, int h)
{
   PACKFILE *f;
   int id;
   ...
   if ((stricmp(get_extension(filename), "bmp") == 0) ||
      ...
      return import_bitmap_font(filename);
   }
   else {
      f = pack_fopen(filename, F_READ);
      if (!f)
...
void save_font(DATAFILE *dat, int packed, int packkids, int strip, int verbose, int extra, PACKFILE *f)
{
   FONT *font = (FONT *)dat->dat;
   BITMAP *bmp;
   int c, x, y;
...
void get_sample_desc(DATAFILE *dat, char *s)
{
   ...
   sprintf(s, "sample (freq %d, %ld.%ld sec)", sample->freq, sec/10, sec%10);
}
...
void *grab_sample(char *filename, long *size, int x, int y, int w, int h)
{
...
void save_sample(DATAFILE *dat, int packed, int packkids, int strip, int verbose, int extra, PACKFILE *f)
{
   SAMPLE *spl = (SAMPLE *)dat->dat;
   ...
   pack_fwrite(spl->data, spl->len, f);
}
...
void *grab_midi(char *filename, long *size, int x, int y, int w, int h)
{
...
void save_midi(DATAFILE *dat, int packed, int packkids, int strip, int verbose, int extra, PACKFILE *f)
{
   MIDI *midi = (MIDI *)dat->dat;
   int c;
...
void get_bitmap_desc(DATAFILE *dat, char *s)
{
   ...
   sprintf(s, "bitmap (%dx%d)", bmp->w, bmp->h);
}
...
void *grab_bitmap(char *filename, long *size, int x, int y, int w, int h)
{
   PALLETE pal;
   BITMAP *bmp;
...
void save_datafile_bitmap(DATAFILE *dat, int packed, int packkids, int strip, int verbose, int extra, PACKFILE *f)
{
   BITMAP *bmp = (BITMAP *)dat->dat;
   int x, y;
...
void get_rle_sprite_desc(DATAFILE *dat, char *s)
{
   ...
   sprintf(s, "RLE sprite (%dx%d)", spr->w, spr->h);
}
...
void *grab_rle_sprite(char *filename, long *size, int x, int y, int w, int h)
{
   BITMAP *bmp;
   RLE_SPRITE *spr;

   bmp = (BITMAP *)grab_bitmap(filename, size, x, y, w, h);
   if (!bmp)
      return NULL;
...
void save_rle_sprite(DATAFILE *dat, int packed, int packkids, int strip, int verbose, int extra, PACKFILE *f)
{
   RLE_SPRITE *spr = (RLE_SPRITE *)dat->dat;
...
void get_c_sprite_desc(DATAFILE *dat, char *s)
{
   ...
   sprintf(s, "compiled sprite (%dx%d)", bmp->w, bmp->h);
}
...
void *grab_pallete(char *filename, long *size, int x, int y, int w, int h)
{
   RGB *pal;
   BITMAP *bmp;
...
/* loads a datafile */
DATAFILE *datedit_load_datafile(char *name, int compile_sprites)
{
   char *pretty_name;
   DATAFILE *datafile;
...
/* saves a datafile */
int datedit_save_datafile(DATAFILE *dat, char *name, int strip, int pack, int verbose, int write_msg, int backup)
{
   char *pretty_name;
...
int datedit_save_header(DATAFILE *dat, char *name, char *headername, char *progname, char *prefix, int verbose)
{
   ...
   if (f) {
	 ...
	 if ((tm[c] == '\r') || (tm[c] == '\n'))
      ...
      fprintf(f, "/* Allegro data file object indexes, produced by %s v" ALLEGRO_VERSION_STR " */\n", progname);
...
DATAFILE *datedit_grab(char *filename, char *name, int type, int x, int y, int w, int h)
{
   static DATAFILE dat;
   void *(*grab)(char *filename, long *size, int x, int y, int w, int h) = NULL;
   ...
   dat.dat = grab(filename, &dat.size, x, y, w, h);
...
int datedit_grabreplace(DATAFILE *dat, char *filename, char *name, char *type)
{
   DATAFILE *tmp = datedit_grab(filename, name, 
				datedit_clean_typename(type), -1, -1, -1, -1);
...
{
   DATAFILE *tmp = datedit_grab(filename, "dummyname", dat->type, x, y, w, h);
...
DATAFILE *datedit_grabnew(DATAFILE *dat, char *filename, char *name, char *type)
{
   DATAFILE *tmp = datedit_grab(filename, name, 
				datedit_clean_typename(type), -1, -1, -1, -1);
   int len;
...
int get_numprop(DATAFILE *dat, int type)
{
...
int datedit_update(DATAFILE *dat, int verbose, int *changed)
{
   ...
   return datedit_grabupdate(dat, origin, 
		     get_numprop(dat, DAT_XPOS), get_numprop(dat, DAT_YPOS), 
		     get_numprop(dat, DAT_XSIZ), get_numprop(dat, DAT_YSIZ));
}
