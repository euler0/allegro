...
void usage()
{
   ...
   fprintf(stderr, "\t'-p prefixstring' sets the object name prefix\n");
...
void output_bitmap(BITMAP *bmp, char *name, int global)
{
   ...
   output_data(bmp->line[0], bmp->w * bmp->h, buf, "bitmap data", FALSE);
   ...
   fprintf(outfile, "\t.long __linear_vtable\n");
...
void output_compiled_sprite(COMPILED_SPRITE *sprite, char *name)
{
   ...
   fprintf(outfile, ".align 4\n_%s%s:\n", prefix, name);
   fprintf(outfile, "\t.long %-16d# planar\n", sprite->planar);
...
int main(int argc, char *argv[])
{
   ...
   data = datedit_load_datafile(infilename, TRUE);
   if (!data) {
      fprintf(stderr, "\nError reading %s\n", infilename);
      err = 1; 
      goto ohshit;
