...
void blur_12(BITMAP *bmp, BITMAP *back)
{
   ...
   for (y=0;y<bmp->h;y++)
   {
      ...
      {
	 // First get the green/blue pixel from each bitmap
	 c1 = bmp->line[y][x];
	 c2 = back->line[y][x];
	 b1 = c1>>4;
	 g1 = c1&15;
	 ...
	 bmp->line[y][x] = g1 + (b1<<4); 

	 // Now do it all again for the red pixel. 
	 c1 = bmp->line[y][x+1];
	 c2 = back->line[y][x+1];
	 r1 = c1&15;
	 r2 = c2&15;
...
int main(int argc, char *argv[])
{
   ...
   {
      printf("Error setting %ix%ix12 (really %ix%ix8, but we fake it):\n%s\n", GFXW, GFXH, GFXW*2, GFXH, allegro_error);
      return 1;
   }

   // Then set your magic palette. From now one you can't use the set_color
   ...
   screen->w /= 2; // because we use 2-bytes per pixel
