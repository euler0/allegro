...
void animate ()
{
   ...
   if ( frame_number == 9 ) { frame_number = 0; } { frame_number++; }
}
...
int main (int argc, char *argv[])
{
   ...
   strcpy(get_filename(datafile_name), "running.dat");

   /* Check if datafile file exists in current directory.                        */
   if ( (file_exists (datafile_name, 32, file_attr) ) == 0)
   {
   printf ("Data file %s not found. Exiting",datafile_name);
   /* exit program */
   return 0;
   }
