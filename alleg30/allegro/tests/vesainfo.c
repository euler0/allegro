 ...
 *      England, TF9 1AJ.
 *
 *      Allegro library utility for checking which video modes are available.
 *
...
int get_mode_info(int mode)
{
   ...
   __dpmi_int(0x10, &r);
   if (r.h.ah) {
      fprintf(stderr, "Int 0x10, ax=0x4F01 failed! ax=0x%02X\n", r.x.ax);
      return -1;
   }
   ...
   if (verbose) {
      ...
      if (mode_info.ModeAttributes & 4) print_flag("BIOS");
      ...
      if (mode_info.ModeAttributes & 32) print_flag("non VGA"); else print_flag("VGA");
      if (!(mode_info.ModeAttributes & 64)) print_flag("banked");
