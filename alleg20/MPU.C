 ...
 *
 *      Marcel de Kogel (m.dekogel@student.utwente.nl) fixed my original
 *      broken version, so that it now actually works :-)
 *
 ...
 */
static int wait_for_mpu(int flag, int port)
{
   int timeout;
...
static END_OF_FUNCTION(wait_for_mpu);
