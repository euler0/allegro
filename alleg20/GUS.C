 ...
 *       written yet. Jesse Morris (cuth@gryphon.ccs.brandeis.edu, 
 *       jrmorris@macalstr.edu) has offered to do some work on this.
 */
...
static void gus_key_on(int voice, int inst, int note, int bend, int vol, int pan);
...
MIDI_DRIVER midi_gus =        /* GUS driver for playing MIDI music */
{
   "Gravis Ultrasound", 
   gus_desc,
   30,               /* This is the number of voices the MIDI player can
		      * use before it has to start cutting off old notes.
		      * Am I right in thinking that 32 voices minus two
		      * for stereo sfx = 30? Or is there some reason we
		      * should use less? The Adlib code can in best case
		      * get 18 voices, so that would be 'enough'.
		      */
   gus_detect,
   gus_midi_init,
   gus_midi_exit,
...
/* gus_key_on:
 *  Triggers the specified MIDI voice (will be passed voice numbers from 0
 *  up to one less than the max voices you specified in the MIDI_DRIVER 
 *  structure.  The instrument is specified as a GM patch number. Drums
 *  are indicated by passing an instrument greater than 128, in which case
 *  you should trigger the GM percussion sound normally mapped to drum key 
 *  (inst-128). Drum sounds can ignore the pitch, bend, and pan parameters,
 *  but should respond to volume. The pitch is a midi note number 
 *  (60=middle C, I think...). Pan and volume are from 0 (left/min) to 
 *  127 (right/max). The bend isn't a midi pitch bend value: it ranges from
 *  0 (normal pitch) to 0xFFF (just a fraction flatter than a semitone up).
 */
static void gus_key_on(int voice, int inst, int note, int bend, int vol, int pan)
{
