...
static FONT *read_font_8x8(PACKFILE *f)
{
   ...
   p->flag_8x8 = TRUE;
   _go32_dpmi_lock_data(p, sizeof(FONT));
   return p;
}
...
static FONT *read_font_prop(PACKFILE *f)
{
   ...
   _go32_dpmi_lock_data(p, sizeof(FONT));
   _go32_dpmi_lock_data(p->dat.dat_prop, sizeof(FONT_PROP));
   return p;
}
...
static RGB *read_pallete(PACKFILE *f, int size)
{
   ...
   _go32_dpmi_lock_data(p, sizeof(PALLETE));
   return p;
}
...
/* read_rle_sprite:
 *  Reads a RLE compressed sprite from a file, allocating memory for it. 
 */
static RLE_SPRITE *read_rle_sprite(PACKFILE *f)
{
   ...
   _go32_dpmi_lock_data(s, sizeof(RLE_SPRITE) + size);
   return s;
}
...
DATAFILE *load_datafile(char *filename)
{
   ...
   _go32_dpmi_lock_data(dat, sizeof(DATAFILE)*(size+1));
