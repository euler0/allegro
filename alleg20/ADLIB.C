 ...
 *      These routines must be compiled with optimisation turned on.
 *
...
static void fm_key_on(int voice, int inst, int note, int bend, int vol, int pan);
...
static int fm_drum_cached_pitch[5];
 ...
 */
static inline void fm_set_drum_pitch(int voice, int note)
{
   int oct = 1;
   int freq, key;
   ...
   fm_write(0xB0+VOICE_OFFSET(voice), key & 0x1F);
}
...
static inline void fm_trigger_drum(int inst, int vol)
{
   ...
   if (fm_drum_op1[d]) {
      if (fm_drum_cached_inst1[d] != fm_drum[inst].instrument) {
	 fm_drum_cached_inst1[d] = fm_drum[inst].instrument;
	 fm_set_drum_op1(fm_drum_channel[d], fm_drum[inst].instrument);
      }
   ...
   if (fm_drum_op2[d]) {
      if (fm_drum_cached_inst2[d] != fm_drum[inst].instrument) {
	 fm_drum_cached_inst2[d] = fm_drum[inst].instrument;
	 fm_set_drum_op2(fm_drum_channel[d], fm_drum[inst].instrument);
      }
   ...
   if ((fm_drum_pitch[d]) && (fm_drum_cached_pitch[d] != drum->pitch)) {
      fm_drum_cached_pitch[d] = drum->pitch;
      fm_set_drum_pitch(fm_drum_channel[d], drum->pitch);
   }
...
static int fm_init()
{
   ...
   LOCK_VARIABLE(fm_drum_cached_pitch);
